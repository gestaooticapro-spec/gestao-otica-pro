...
import { CheckCircle2, Filter, Loader2, X, ArrowRight, ArrowDownAZ } from 'lucide-react'
...
export default function ContratosQuitadosModal({ storeId, onClose }: { storeId: number; onClose: () => void }) {
    ...
    const [contratosCarregados, setContratosCarregados] = useState<ContratoQuitado[]>([])
    const [loading, setLoading] = useState(true)
    const [error, setError] = useState('')
    const [ordenacao, setOrdenacao] = useState<'data' | 'cliente'>('data')
    ...
    const buscar = async (inicio = dataInicial, fim = dataFinal) => {
        ...
        if (result.success) setContratosCarregados(result.data)
    ...
    const contratosOrdenados = useMemo(() => {
        if (ordenacao === 'data') return contratosCarregados

        return [...contratosCarregados].sort((a, b) => {
            const porCliente = a.cliente_nome.localeCompare(b.cliente_nome, 'pt-BR', { sensitivity: 'base' })
            return porCliente || b.data_quitacao.localeCompare(a.data_quitacao)
        })
    }, [contratosCarregados, ordenacao])
    ...
    return <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 backdrop-blur-md p-4" onClick={onClose}>
        <div className="w-full max-w-5xl max-h-[90vh] overflow-hidden rounded-2xl border border-white/10 bg-slate-900 shadow-2xl flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-wrap items-end gap-3 border-b border-white/10 bg-slate-900 px-6 py-4">
                ...
                <button onClick={() => setOrdenacao((atual) => atual === 'cliente' ? 'data' : 'cliente')} disabled={loading || contratosCarregados.length === 0} className="flex h-10 items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 text-xs font-bold text-slate-300 hover:bg-white/10 hover:text-white disabled:cursor-not-allowed disabled:opacity-50" title={ordenacao === 'cliente' ? 'Voltar à ordem por data' : 'Ordenar por cliente'}><ArrowDownAZ className="h-4 w-4" />{ordenacao === 'cliente' ? 'Ordem por data' : 'Ordem alfabética'}</button>
            </div>
