...
export function WidgetLaboratorio({ data, storeId }: { data: AlertaLaboratorio[], storeId: number }) {
    ...
    return (
        <div className="flex flex-col bg-white rounded-3xl shadow-[0_10px_30px_-10px_rgba(225,29,72,0.15)] border border-white/50 overflow-hidden h-fit transition-all duration-300">
            <div
                className="px-6 py-5 flex justify-between items-center cursor-pointer hover:bg-rose-50/30 transition-colors"
            ...
            {isOpen && (
                <div className="p-4 pt-0 overflow-y-auto custom-scrollbar space-y-3 max-h-[400px] animate-in slide-in-from-top-2 duration-200">
                    {data.length === 0 ? (
...
export function WidgetEntregas({ data, storeId }: { data: AlertaEntrega[], storeId: number }) {
    ...
    return (
        <div className="flex flex-col bg-white rounded-3xl shadow-[0_10px_30px_-10px_rgba(79,70,229,0.15)] border border-white/50 overflow-hidden h-fit transition-all duration-300">
            <div
                className="px-6 py-5 flex justify-between items-center cursor-pointer hover:bg-indigo-50/30 transition-colors"
                onClick={() => setIsOpen(!isOpen)}
            >
                <div className="flex items-center gap-3">
