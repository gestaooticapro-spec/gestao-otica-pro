...
import { ArrowRight, Check, Clock3, ExternalLink, GitMerge, Loader2, ShieldAlert, Trash2, X } from 'lucide-react'
...
function CustomerRecord({ record }: { record: DuplicateRecord }) {
  return <article className="min-w-0 border border-white/10 bg-black/20 p-4">
    <h4 className="truncate font-bold text-white">{record.full_name}</h4>
...
function ProductRecord({ record }: { record: DuplicateRecord }) {
  return <article className="min-w-0 border border-white/10 bg-black/20 p-4">
    <h4 className="font-bold text-white">{record.nome}</h4>
    <dl className="mt-3 space-y-1 text-sm text-slate-300">
  ...
  return <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/85 p-4 backdrop-blur-sm" onClick={onClose}>
    <section className="max-h-[90vh] w-full max-w-6xl overflow-y-auto border border-white/10 bg-slate-950 p-5 shadow-2xl md:p-7" onClick={(event) => event.stopPropagation()}>
      ...
      {!loading && duplicate && data?.groups?.length ? <div className="mt-5 space-y-6">{data.groups.map((group, index) => {
        ...
        <div className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm font-bold text-white">Caso {index + 1} de {data.groups?.length}</p><p className="text-xs text-amber-200">Coincidencia: {group.reasons.map(reasonLabel).join(', ')}</p></div>
        <div className={`mt-3 grid gap-3 ${group.records.length === 2 ? 'md:grid-cols-2' : 'md:grid-cols-3'}`}>{group.records.map((record) => kind === 'duplicate-customers' ? <CustomerRecord key={record.id} record={record} /> : <ProductRecord key={record.id} record={record} />)}</div>
        <div className="mt-3 flex flex-wrap gap-2">{group.records.map((record) => {
          const previewKey = `${group.fingerprint}:${record.id}`
          const label = kind === 'duplicate-customers' ? record.full_name : record.nome
          return <button key={record.id} type="button" disabled={loadingPreview !== null || saving !== null} onClick={() => void loadMergePreview(group, record.id)} className="inline-flex h-9 items-center gap-2 border border-cyan-300/30 px-3 text-xs font-semibold text-cyan-100 hover:bg-cyan-300/10 disabled:opacity-50">{loadingPreview === previewKey ? <Loader2 className="h-4 w-4 animate-spin" /> : <GitMerge className="h-4 w-4" />}Prévia usando {label || `#${record.id}`}</button>
        })}</div>
        {preview ? <section className="mt-4 border border-cyan-300/20 bg-cyan-300/5 p-4" aria-label="Prévia da mesclagem">
          ...
          {preview.dependencies.length ? <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{preview.dependencies.map((dependency) => <div key={`${dependency.table}:${dependency.label}`} className="border border-white/10 bg-black/20 px-3 py-2"><p className="text-xs text-slate-400">{dependency.label}</p><p className="mt-1 text-sm font-bold text-white">{dependency.sourceCount} a mover <span className="font-normal text-slate-500">· {dependency.targetCount} já no principal</span></p></div>)}</div> : <p className="mt-4 text-sm text-slate-300">Nenhum vínculo operacional precisa ser transferido.</p>}
