...
const RecommendationCaseSchema = z.object({
  ...
  receita: z.object({
    od: z.object({ esferico: z.number().finite().min(-30).max(30).nullable(), cilindrico: z.number().finite().min(-15).max(15).nullable(), eixo: z.number().finite().min(0).max(180).optional().nullable() }),
    oe: z.object({ esferico: z.number().finite().min(-30).max(30).nullable(), cilindrico: z.number().finite().min(-15).max(15).nullable(), eixo: z.number().finite().min(0).max(180).optional().nullable() }),
    olhos_utilizaveis: z.enum(['ambos', 'od', 'oe']).optional(),
  }).optional(),
