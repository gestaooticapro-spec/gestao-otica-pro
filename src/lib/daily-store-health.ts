...
function buildPostSaleAnalysis(postSales: any[], followups: any[], interactions: any[], serviceOrders: any[], inboundMessages: any[]): PostSaleAnalysis {
  ...
  for (const postSale of postSales) {
    ...
    if (summaries.some((summary) => /reclam|adapta̤̣o|adaptacao/i.test(summary))) {
...
const DATA_SOURCE_ALERTS: Array<{ id: string; area: DailyHealthArea; sources: string[]; title: string; detail: string }> = [
  ...
  },
  {
    ...
    area: 'operacao',
    sources: ['ordens de servico', 'vendas de lentes', 'lentes das OS'],
...
function fallbackAreaNarratives(metrics: DailyHealthMetrics, alerts: DailyHealthAlert[] = []): DailyHealthAreaNarratives {
  ...
  return {
    ...
    relacionamento: hasUnavailableData('relacionamento') ? 'Nao foi possivel atualizar todos os dados de relacionamento. O relatorio nao concluiu que pendencias anteriores foram resolvidas.' : changed('relacionamento') ? relationshipFacts.join(' ') : quiet('relacionamento'),
...
function groundedAiText(value: unknown, fallbackValue: string, evidence: string[]) {
  return isGroundedAiText(value, evidence) ? String(value).trim() : fallbackValue
}
  ...
  try {
    ...
    return {
      ...
      areas: {
        ...
        relacionamento: !dadosIndisponiveis('relacionamento') && relacionamentoTemMudanca ? groundedAiText(parsed.relacionamento, fallback.areas.relacionamento, relationshipEvidence) : fallback.areas.relacionamento,
...
export async function generateDailyStoreHealthReport(storeId = 1, reportDate = previousDateKey(dateKey(), 1)): Promise<DailyHealthReport> {
  ...
  if (isReadySnapshot(existingReport)) return reportFromStoredRow(existingReport)
  ...
  const [storeResult, settingsResult, salesResult, monthSalesResult, lastYearSalesResult, paymentsResult, monthPaymentsResult, lastYearPaymentsResult, financingsResult, monthFinancingsResult, lastYearFinancingsResult, installmentsResult, accountsPayableResult, ordersResult, lensSalesResult, postSalesResult, postSalesCompletedYesterdayResult, postSalesCompletedWeekResult, postSaleFollowupsResult, postSaleInteractionsResult, postSaleOrdersResult, itemsResult, whatsAppResult] = await Promise.allSettled([
    ...
    loadAllRows((from, to) => (admin.from('vendas') as any).select('id,valor_final,valor_restante,financiamento_loja!financiamento_loja_venda_id_fkey(valor_total_financiado)').eq('store_id', storeId).eq('status', 'Fechada').gte('created_at', target.start).lte('created_at', target.end).range(from, to)),
    loadAllRows((from, to) => (admin.from('vendas') as any).select('id,valor_final,valor_restante,financiamento_loja!financiamento_loja_venda_id_fkey(valor_total_financiado)').eq('store_id', storeId).eq('status', 'Fechada').gte('created_at', monthTarget.start).lte('created_at', monthTarget.end).range(from, to)),
    loadAllRows((from, to) => (admin.from('vendas') as any).select('id,valor_final,valor_restante,financiamento_loja!financiamento_loja_venda_id_fkey(valor_total_financiado)').eq('store_id', storeId).eq('status', 'Fechada').gte('created_at', lastYearTarget.start).lte('created_at', lastYearTarget.end).range(from, to)),
    ...
    loadAllRows((from, to) => (admin.from('service_orders') as any).select('id,venda_id,created_at,dt_prometido_para,dt_pedido_em,dt_lente_chegou,dt_montado_em,dt_montado_no_lab,dt_recebido_na_loja,dt_entregue_em,lab_encerrada_em,os_enviada_ao_lab,armacao_com_cliente,receita_longe_od_esferico,receita_longe_od_cilindrico,receita_longe_oe_esferico,receita_longe_oe_cilindrico,receita_perto_od_esferico,receita_perto_od_cilindrico,receita_perto_oe_esferico,receita_perto_oe_cilindrico,links:venda_itens_os_links(venda_item_id,uso_na_os),vendas(status)').eq('store_id', storeId).range(from, to)),
    loadAllRows((from, to) => (admin.from('venda_itens') as any).select('venda_id,products!inner(tipo_produto),vendas!inner(status,created_at)').eq('store_id', storeId).eq('products.tipo_produto', 'Lente').eq('vendas.status', 'Fechada').gte('vendas.created_at', rangeForDate(previousDateKey(reportDate, 30)).start).lte('vendas.created_at', target.end).range(from, to)),
    loadAllRows((from, to) => (admin.from('post_sales') as any).select('id,status,service_order_id,avaliacao_cliente').eq('store_id', storeId).neq('status', 'Concluido').range(from, to)),
    ...
    loadAllRows((from, to) => (admin.from('venda_itens') as any).select('id,product_id,valor_total_item,products(preco_custo),vendas!inner(created_at,status)').eq('store_id', storeId).eq('vendas.status', 'Fechada').gte('vendas.created_at', target.start).lte('vendas.created_at', target.end).range(from, to)),
  ...
  const { data: saved, error } = await (admin.from('daily_store_health_reports') as any).insert({ tenant_id: store.tenant_id, store_id: storeId, report_date: reportDate, status: 'ready', metrics: report.metrics, alerts: report.alerts, narrative: report.narrative, source_failures: report.sourceFailures, generated_at: report.generatedAt, generation_started_at: report.generatedAt, updated_at: report.generatedAt }).select('id').single()
  if (error) {
    if (error.code === '23505') {
      const { data: concurrentReport, error: concurrentReportError } = await (admin.from('daily_store_health_reports') as any).select('*').eq('store_id', storeId).eq('report_date', reportDate).maybeSingle()
