...
// 1. Remove ANY existing TEST_PROFILES blocks (greedy clean)
content = content.replace(/\/\/ =*[\s\S]*?\/\/ TEST PROFILES \(FOR DEMO\)[\s\S]*?const TEST_PROFILES = \{[\s\S]*?\}\;\n/g, '');
// And another variant if the comments change
content = content.replace(/\/\/ =*[\s\S]*?\/\/ =*\nconst TEST_PROFILES = \{[\s\S]*?\}\;\n/g, '');

// 2. Remove ANY existing UI Row blocks
content = content.replace(/\{\/\* TEST PROFILES - REMOVE EASILY BY DELETING THIS BLOCK \*\/\}[\s\S]*?<div className="flex flex-wrap items-center[\s\S]*?<\/div>\n/g, '');

// 3. Clean encoding (just in case)
...
// 4. Inject CLEAN TEST_PROFILES after getParseStatusLabel
const cleanProfiles = `
// ==========================================
...
content = content.replace(
  /const getParseStatusLabel = \(value: EvaluationParseStatus\) => \{[\s\S]*?\}\n\}/,
  (match) => match + cleanProfiles
);

// 5. Inject CLEAN UI Row
const cleanUI = `
...
console.log('Cleanup and Injection complete.');
