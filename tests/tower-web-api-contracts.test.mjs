...
test('sync do dispositivo aceita todo o atendimento offline e a configuracao instala dados operacionais', async () => {
  ...
  assert.match(configuration, /export const revalidate = 0/)
  assert.match(configuration, /export const fetchCache = 'force-no-store'/)
  assert.match(configuration, /createAdminClient\(\{ noStore: true \}\)/)
