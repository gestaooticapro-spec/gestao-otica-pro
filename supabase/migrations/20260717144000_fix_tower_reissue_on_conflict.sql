-- O nome store_id tambem existe como coluna de retorno da funcao. Em PL/pgSQL,
-- ON CONFLICT (store_id) fica ambiguo entre essa variavel implicita e a coluna
-- da tabela. Referenciar a constraint elimina a ambiguidade.
CREATE OR REPLACE FUNCTION public.reissue_tower_store_activation(
    p_store_id BIGINT,
    p_token_hash TEXT,
...
BEGIN
    ...
    SET status = 'revoked',
        revoked_at = NOW()
    WHERE activation.store_id = p_store_id
      AND activation.status = 'pending';
