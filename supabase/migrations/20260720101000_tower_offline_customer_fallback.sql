-- Fallback de cliente provisório para a Torre.
-- O UUID nasce no SQLite e e reconciliado com o BIGINT canonico de customers.
...
CREATE TABLE IF NOT EXISTS public.tower_device_customer_mappings (
    ...
    PRIMARY KEY (device_id, local_customer_id)
);

CREATE INDEX IF NOT EXISTS idx_tower_device_customer_mappings_customer
    ON public.tower_device_customer_mappings(customer_id);

ALTER TABLE public.tower_device_customer_mappings ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON TABLE public.tower_device_customer_mappings FROM PUBLIC, anon, authenticated;
...
BEGIN
        ...
        IF resolved_customer_id IS NULL THEN
            ...
            IF existing_customer_id IS NULL THEN
                INSERT INTO public.customers(
                    tenant_id, store_id, full_name, fone_movel, created_at
                ) VALUES (
                    resolved_tenant_id, resolved_store_id,
            ...
            INSERT INTO public.tower_device_customer_mappings(
                ...
                p_device_id, p_entity_id, resolved_tenant_id,
                resolved_store_id, resolved_customer_id
    ...
    IF p_event_type = 'tower_session.upsert' THEN
        local_customer_id := NULLIF(p_payload->>'localCustomerId', '')::UUID;
        IF local_customer_id IS NOT NULL THEN
            SELECT mapping.customer_id INTO resolved_customer_id
            FROM public.tower_device_customer_mappings AS mapping
            WHERE mapping.device_id = p_device_id
              AND mapping.local_customer_id = local_customer_id
