-- Mantem a baixa atomica alinhada ao saldo efetivo exibido pela aplicacao:
-- nominal + transferido_entrada - pago - transferido_saida - renegociado_saida.
-- As funcoes publicas continuam delegando para estas versoes internas.

create or replace function public.receive_installment_payment_core(
...
begin
  ...
  from public.financiamento_parcelas where id = p_installment_id;
  if v_financing_id is null then raise exception 'Parcela nao encontrada.'; end if;

  perform id from public.financiamento_parcelas
  where financiamento_id = v_financing_id
  order by numero_parcela, id for update;

  select * into v_origin from public.financiamento_parcelas where id = p_installment_id;
  select * into v_financing from public.financiamento_loja where id = v_origin.financiamento_id;
  if not found or v_financing.venda_id <> p_sale_id then raise exception 'Carne e venda nao correspondem.'; end if;
  ...
  if v_origin.store_id <> p_store_id or v_origin.tenant_id is distinct from p_tenant_id then
  ...
  v_origin_balance := greatest(0, round(
    ...
    - coalesce(v_origin.valor_transferido_saida, 0)
    - coalesce(v_origin.valor_renegociado_saida, 0), 2
  ));
  if lower(coalesce(v_origin.status, '')) <> 'pendente' or v_origin_balance <= 0 then
    raise exception 'Esta parcela ja esta quitada.';
  end if;
  if v_principal <= 0 then raise exception 'Informe um valor de principal para abater a parcela.'; end if;
  ...
  if v_strategy = 'somar_proxima' then
    select * into v_next from public.financiamento_parcelas
    where financiamento_id = v_origin.financiamento_id
      ...
      and greatest(0, round(
        coalesce(valor_parcela, 0)
        + coalesce(valor_transferido_entrada, 0)
        - coalesce(valor_pago, 0)
        - coalesce(valor_transferido_saida, 0)
        - coalesce(valor_renegociado_saida, 0), 2
      )) > 0
    order by numero_parcela, id limit 1;
    if not found then raise exception 'Nao existe proxima parcela para receber o saldo restante.'; end if;
  end if;

  select coalesce(jsonb_agg(to_jsonb(fp) order by fp.numero_parcela, fp.id), '[]'::jsonb)
    into v_installments_before from public.financiamento_parcelas fp
  ...
  if v_strategy = 'somar_proxima' then
    ...
    set valor_pago = round(coalesce(valor_pago, 0) + v_principal, 2),
        ...
        status = 'Pago', data_pagamento = p_received_on
    ...
    v_allocations := jsonb_build_array(jsonb_build_object(
      'id', v_origin.id, 'numero_parcela', v_origin.numero_parcela,
    ...
    loop
      ...
      v_installment_balance := greatest(0, round(
        ...
        - coalesce(v_installment.valor_transferido_saida, 0)
        - coalesce(v_installment.valor_renegociado_saida, 0), 2
      ));
      ...
      v_allocations := v_allocations || jsonb_build_array(jsonb_build_object(
        'id', v_installment.id, 'numero_parcela', v_installment.numero_parcela,
    ...
    if v_remaining > 0 then raise exception 'O valor principal ultrapassa o saldo das parcelas deste carne.'; end if;
  end if;
  ...
  loop
    ...
    while v_allocation_remaining > 0 loop
      if v_receipt is null then raise exception 'Nao foi possivel distribuir as formas de recebimento.'; end if;
  ...
  if v_sale.is_historical_import is true then
    select round(coalesce(sum(greatest(0,
      coalesce(valor_parcela, 0) + coalesce(valor_transferido_entrada, 0)
      - coalesce(valor_pago, 0) - coalesce(valor_transferido_saida, 0)
      - coalesce(valor_renegociado_saida, 0)
    )), 0), 2) into v_historical_balance
    from public.financiamento_parcelas where financiamento_id = v_origin.financiamento_id;
    update public.vendas set valor_restante = v_historical_balance, status = 'Fechada'
    where id = p_sale_id and store_id = p_store_id;
  end if;

  select coalesce(jsonb_agg(to_jsonb(fp) order by fp.numero_parcela, fp.id), '[]'::jsonb)
    into v_installments_after from public.financiamento_parcelas fp
  where fp.financiamento_id = v_origin.financiamento_id;
  select to_jsonb(v) into v_sale_after from public.vendas v where id = p_sale_id;
  select count(*) into v_affected_count from jsonb_array_elements(v_installments_after) after_item
  ...
  set installments_after = v_installments_after, sale_after = v_sale_after,
      payments_created = v_payments, affected_installment_count = greatest(1, v_affected_count),
      transferred_amount = v_transfer, destination_installment_id = v_destination_id,
      state = 'completed', completed_at = now()
  where id = v_operation_id;

  return jsonb_build_object(
    'operation_id', v_operation_id, 'payment_ids', v_payment_ids,
    'receipt_installment_ids', v_installment_ids, 'strategy', v_strategy,
    'transferred_amount', v_transfer, 'destination_installment_id', v_destination_id
  );
end;
$$;

create or replace function public.reverse_installment_receipt_operation_internal(
...
begin
  ...
  select * into v_operation from public.installment_receipt_operations
  ...
  select * into v_manager from public.employees
  where id = p_authorizing_employee_id and store_id = v_operation.store_id
    and is_active = true and lower(role) = 'gerente';
  if not found then raise exception 'A reversao exige autorizacao de um gerente ativo da loja.'; end if;
  select id into v_newer_operation_id from public.installment_receipt_operations
  where financiamento_id = v_operation.financiamento_id and state = 'completed'
    and reversed_at is null and (created_at, id) > (v_operation.created_at, v_operation.id)
  order by created_at desc, id desc limit 1;
  if v_newer_operation_id is not null then raise exception 'Reverta primeiro o recebimento mais recente deste carne.'; end if;
  ...
  loop
    ...
    set numero_parcela = (v_snapshot->>'numero_parcela')::integer,
        ...
        valor_renegociado_saida = coalesce(nullif(v_snapshot->>'valor_renegociado_saida', '')::numeric, 0),
        data_pagamento = nullif(v_snapshot->>'data_pagamento', '')::date,
        status = v_snapshot->>'status', customer_id = nullif(v_snapshot->>'customer_id', '')::bigint,
        obs = v_snapshot->>'obs'
    where id = (v_snapshot->>'id')::bigint and financiamento_id = v_operation.financiamento_id;
  end loop;
  ...
  update public.installment_receipt_operations
  set state = 'reversed', reversed_at = now(), reversed_by_employee_id = p_authorizing_employee_id,
      reversed_by_user_id = p_user_id, reversal_reason = trim(p_reason)
  where id = v_operation.id;
  return jsonb_build_object('operation_id', v_operation.id,
    'origin_installment_id', v_operation.origin_installment_id, 'payment_ids', v_payment_ids);
end;
$$;
