...
if (!versionId) {
  console.error('Uso: node scripts/fix_optilab_page12_varilux_xrtrack.js --version-id=UUID [--commit]')
  process.exit(1)
}
...
async function main() {
  // Evidence (page 12 - Optilab PVC Digital v2):
  // - Alt. Mínimo: 14mm
  // - Cilíndrico até -6.00
  // - Adição: 1.00 a 3.50 (Stylis 1.74: 1.00 a 4.00)
  // - Missing row: XR Track Airwear + Coloração especial (Verniz HC = 6.479,00)
  const heightTarget = 14
  const stylis174AddMax = 4
  const missingAirwearColoracaoEspecialPrice = 6479
  ...
  const xrTrackOffers = (offers || []).filter((o) => {
    const t = noAcc(`${o.canonical_label || ''} ${o.raw_label || ''}`)
    return t.includes('varilux') && t.includes('xr') && t.includes('xr track') && !t.includes('lite')
  })

  if (!xrTrackOffers.length) {
    console.log('Nenhuma oferta XR Track encontrada.')
    return
  }

  const offerIds = xrTrackOffers.map((o) => o.id)
  ...
  // 1) Apply min_fitting_height=14 to all XR Track offers missing it
  const heightTargets = xrTrackOffers.filter((o) => ensureObject(o.features).min_fitting_height == null)

  // 2) Fix Stylis 1.74 add_max=4 for XR Track (two rows: with and without Gen S)
  const stylis174Targets = xrTrackOffers.filter((o) => {
    const t = noAcc(o.canonical_label || o.raw_label)
    return t.includes('stylis') && t.includes('1.74') && t.includes('xr track') && !t.includes('solares')
  })
  ...
  const airwearColorPadrao = xrTrackOffers.find((o) => {
    const t = noAcc(o.canonical_label || o.raw_label)
    return (
      t.includes('solares') &&
  ...
  const airwearColorEspecialExisting = xrTrackOffers.find((o) => {
    const t = noAcc(o.canonical_label || o.raw_label)
    return t.includes('solares') && t.includes('coloracao') && t.includes('xr track') && t.includes('airwear') && t.includes('coloracao especial')
  })
  ...
  if (needsInsert) {
    if (!airwearColorPadrao) {
      throw new Error('Template "XR Track Airwear (Coloração padrão)" não encontrado. Não consigo inserir coloração especial com segurança.')
    }

    const base = airwearColorPadrao
    const newCanonical = 'VARILUX® XR SERIES SOLARES / COLORAÇÃO XR Track Airwear + Coloração especial'
    const newRaw = base.raw_label ? `${base.raw_label} + Coloração especial` : 'Airwear + Coloração especial'
    const newImportKey = `${base.import_key || 'varilux_xrtrack_airwear'}__coloracao_especial`
    ...
    const offerPayload = {
      ...
      source_page_reference: base.source_page_reference || 'p. 12',
    }
  ...
  if (heightTargets.length) {
    console.log('[height] ofertas XR Track sem min_fitting_height:', heightTargets.length, '=>', heightTarget)
    if (commit) {
      ...
      if (insertedOfferId) {
        const base = airwearColorPadrao
        const nextFeatures = { ...(ensureObject(base.features)), min_fitting_height: heightTarget, coloracao_especial: true }
        await supabase.from('global_lens_offers').update({ features: nextFeatures }).eq('id', insertedOfferId)
      }
    }
  } else {
    console.log('[height] todas as ofertas XR Track já tinham min_fitting_height.')
  }

  // Apply Stylis 1.74 add fix
  if (stylis174NeedsFix.length) {
    console.log('[add] ofertas Stylis 1.74 XR Track com add_max != 4:', stylis174NeedsFix.length)
    ...
    console.log('[add] Stylis 1.74 XR Track já estava com add_max=4.')
  }

  console.log('Resumo:')
  console.log('- XR Track ofertas encontradas:', xrTrackOffers.length)
  console.log('- Altura aplicada:', heightTargets.length, '=>', heightTarget)
  console.log('- Stylis 1.74 (XR Track) ofertas alvo:', stylis174Targets.length)
  console.log('- Stylis 1.74 (XR Track) ofertas corrigidas:', stylis174NeedsFix.length)
