...
if (!versionId) {
  console.error('Uso: node scripts/audit_hoya_pages_38_39_sportive.js --version-id=UUID')
  process.exit(1)
}
...
async function main() {
  ...
  const rows = offers
    .map((o) => {
      ...
      return {
        ...
        clinical_category: o.clinical_category,
        section: o.features?.cor ?? null,
        treatment: o.features?.tratamento ?? null,
        curve: o.features?.curva ?? null,
        min_fitting_height: o.features?.min_fitting_height ?? null,
        fitting_heights_available: o.features?.fitting_heights_available ?? null,
    ...
    .sort(
      (a, b) =>
        ...
        String(a.base_price || '').localeCompare(String(b.base_price || '')) ||
  ...
  const summary = {
    versionId,
    offers: rows.length,
    byPage: rows.reduce((acc, r) => {
      acc[r.page] = (acc[r.page] || 0) + 1
      return acc
    }, {}),
    byFamily: rows.reduce((acc, r) => {
      acc[r.family] = (acc[r.family] || 0) + 1
      return acc
    }, {}),
    cylInvertedCount: rows.filter((r) => r.cyl_inverted).length,
    distinct: {
      sections: Array.from(new Set(rows.map((r) => r.section).filter(Boolean))).sort(),
      treatments: Array.from(new Set(rows.map((r) => r.treatment).filter(Boolean))).sort(),
      materials: Array.from(new Set(rows.map((r) => r.material).filter(Boolean))).sort(),
      curves: Array.from(new Set(rows.map((r) => r.curve).filter(Boolean))).sort(),
      clinical_categories: Array.from(new Set(rows.map((r) => r.clinical_category).filter(Boolean))).sort(),
    },
  }

  console.log('AUDIT_HOYA_P38_P39_SPORTIVE')
