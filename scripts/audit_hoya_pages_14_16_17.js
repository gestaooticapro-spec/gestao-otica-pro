...
if (!versionId) {
  console.error('Uso: node scripts/audit_hoya_pages_14_16_17.js --version-id=UUID')
  process.exit(1)
}
...
async function main() {
  ...
  const rows = offers
    .map((o) => {
      ...
      return {
        ...
        diameter: gs.find((g) => g.metadata && (g.metadata.diametro || g.metadata.diametro_mm || g.metadata.diameter))
          ? gs.find((g) => g.metadata && (g.metadata.diametro || g.metadata.diametro_mm || g.metadata.diameter))
              .metadata
          : null,
  ...
  console.log('AUDIT_HOYA_P14_P16_P17')
  console.log(JSON.stringify({ summary, rows }, null, 2))
}

main().catch((err) => {
  console.error('Erro audit hoya p14/p16/p17:', err)
  process.exit(1)
})
