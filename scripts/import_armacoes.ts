...
async function runImport() {
    ...
    console.log(`Iniciando importação de ${dataLines.length} Armações... ${dryRun ? '[MODO DRY-RUN]' : ''}`)
    ...
    for (const line of dataLines) {
        ...
        // Limpeza de valor (caso venha com ponto como separador de milhar, ex: 1.100)
        ...
        if (dryRun) {
            console.log(`[DRY-RUN] Inserindo Armação: ${payload.nome} | Barcode: ${generatedBarcode} | Preço: ${precoVenda} | Estoque: ${qtde}`)
        } else {
            const { error } = await supabase.from('products').insert(payload)
            if (error) {
                console.error(`Erro ao inserir ${payload.nome}:`, error.message)
            } else {
                console.log(`Inserido com sucesso (Armação): ${payload.nome} (${generatedBarcode})`)
            }
        }
    }
