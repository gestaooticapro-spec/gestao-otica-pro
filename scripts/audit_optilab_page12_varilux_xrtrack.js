...
const outPath =
  ...
  path.join(process.cwd(), '.tabelas', 'auditoria_optilab_page12.json')

if (!versionId) {
  console.error('Uso: node scripts/audit_optilab_page12_varilux_xrtrack.js --version-id=UUID [--out=PATH]')
  process.exit(1)
}
...
async function main() {
  ...
  const report = xrTrackOffers
    .map((o) => ({
      ...
      compatibilities: (compatByOfferId.get(o.id) || []).sort((a, b) =>
        String(a.treatment).localeCompare(String(b.treatment)),
      ),
    }))
    .sort((a, b) => String(a.label).localeCompare(String(b.label)))

  const checks = {
    stylis_174_add_not_4: report.filter((r) => noAcc(r.label).includes('stylis 1.74') && r.grid?.add?.[1] !== 4),
    missing_min_fitting_height: report.filter((r) => r.min_fitting_height == null),
    missing_airwear_coloracao_especial: report.filter((r) =>
      noAcc(r.label).includes('solares') &&
      noAcc(r.label).includes('xr track') &&
      noAcc(r.label).includes('airwear') &&
      noAcc(r.label).includes('coloracao especial'),
    ).length
      ? []
      : ['XR Track Airwear + Coloração especial (ver página 12)'],
  }

  const out = {
    versionId,
    source: 'Optilab PVC Digital v2 - page 12 images',
    xr_track_offer_count: report.length,
    offers: report,
    checks: {
      stylis_174_add_not_4: checks.stylis_174_add_not_4.map((r) => ({
        label: r.label,
        add: r.grid?.add || null,
      })),
  ...
  console.log('XR_TRACK_OFFERS', report.length)
