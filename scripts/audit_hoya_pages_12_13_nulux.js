...
if (!versionId) {
  console.error('Uso: node scripts/audit_hoya_pages_12_13_nulux.js --version-id=UUID')
  process.exit(1)
}
...
async function main() {
  const pages = ['Pagina 12', 'Pagina 13']
  ...
  const offers = await fetchAll(
    supabase
      ...
      .select(
        'id,family_id,canonical_label,raw_label,base_price,material,indice_refracao,features,source_page_reference'
      )
  ...
  const rows = offers
    .map((o) => {
      ...
      return {
        ...
        features: o.features || {},
        grid_count: gs.length,
        sph: rangeFromGrids(gs, 'sph_min', 'sph_max'),
        cyl: rangeFromGrids(gs, 'cyl_min', 'cyl_max'),
        add: rangeFromGrids(gs, 'add_min', 'add_max'),
        cyl_inverted: gs.some((g) => g.cyl_min != null && g.cyl_max != null && Number(g.cyl_min) > Number(g.cyl_max)),
      }
    })
    .sort((a, b) => (a.page || '').localeCompare(b.page || '') || (a.family || '').localeCompare(b.family || ''))
  ...
  console.log('AUDIT_HOYA_P12_P13_NULUX')
  console.log(JSON.stringify({ summary, rows }, null, 2))
}

main().catch((err) => {
  console.error('Erro audit hoya p12/p13:', err)
  process.exit(1)
})
